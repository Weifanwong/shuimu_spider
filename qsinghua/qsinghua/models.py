...
class Topic(Document):
    ...
    summary = StringField(required=False,default='')
...
class Known_Post(Document):
    ...
    meta = {
        'ordering': ['-pt_time'], # 默认的　objects()
        'collection': 'known_post',
        # 'shard_key': ('site_id', 'topic_id', 'pt_time'),
        # 'queryset_class': PostQuerySet,

    }

class Known_Poster(Document):
    home_url = StringField(max_length=512)
    img_url = StringField(max_length=512)
    ...
    meta = {
        'ordering': ['-pt_time'], # 默认的　objects()
        'collection': 'known_poster',

    }

class Event_Hot_Value(Document):
    date = DateTimeField(required=True)
    famous_num = IntField(default=0)
    post_num = IntField(default=0)
    hot_value = FloatField(default=0)
    topic_id = IntField(default=0)
    event_id = IntField(default=0)

    meta = {
        'ordering': ['-date'], # 默认的　objects()
        'collection': 'event_hot_value',

    }

class Event_Topic_Evo(Document):
    date = DateTimeField(required=True)
    topic_id = IntField(default=0)
    topic_kw = StringField(max_length=64)
    kw_volumn = IntField(default=0)
    type_id = IntField(default=0)
    event_id = IntField(default=0)

    meta = {
        'ordering': ['-date'], # 默认的　objects()
        'collection': 'event_topic_evo',

    }

class Point_Mining(Document):

    topic_id = IntField(default=0)
    viewpoint = StringField(max_length=256)
    usercomment = ListField(DictField(),default=list)
    event_id = IntField(default=0)

    meta = {
        'ordering': ['-topic_id'], # 默认的　objects()
        'collection': 'point_mining',

    }

class Emotion_Mining(Document):

    topic_id = IntField(default=0)
    event_id = IntField(default=0)
    _id = ObjectIdField(required=True, unique=True)
    post_url = StringField(max_length=256)
    site_name = StringField(max_length=128)
    title = StringField(max_length=256)
    content = StringField(max_length=1028)
    pt_time = DateTimeField()
    good_num = IntField(default=0)
    comm_num = IntField(default=0)
    img_url = StringField(max_length=128)
    repost_num = IntField(default=0)
    poster_url = StringField(max_length=128)
    poster = StringField(max_length=64)
    user_id = StringField(max_length=64)
    video_url = StringField(max_length=128)
    label = StringField(max_length=64)

    meta = {
        'ordering': ['-topic_id'], # 默认的　objects()
        'collection': 'emotion_mining',

    }
    
class Save_Score(Document):
    _id = ObjectIdField(primary_key=True,unique=True)
    event_id = IntField()
    time = StringField()
    result = IntField()
    fin = BooleanField()
    expert = IntField()
    consistent = FloatField()

    meta = {
        # 'ordering': ['event_id'], # 默认的　objects()
        'collection': 'save_score'
    }
class Save_Strategy(Document):

    event_id = IntField()
    time = StringField(max_length=256)
    strategy1 = StringField(max_length=2048)
    strategy2 = StringField(max_length=2048)

    meta = {
        'ordering': ['event_id'], # 默认的　objects()
        'collection': 'save_strategy',
    }


class Topic_Template(Document):


    exam_type = IntField(required=True, choices=EXAM_TYPE)
    exam_period = IntField(required=True, choices=EXAM_PERIOD)
    topic_name = StringField(max_length=128)
    site_id = IntField(required=True)
    topic_id = IntField(required=True)
    topic_kw = ListField(ListField(StringField(), default=list), default=list)
    topic_kws = ListField(StringField(), default=list)
    user_id = IntField(required=True,unique_with=['topic_id','site_id'])
    user_name = StringField(required=True, max_length=32)
...
class Topic_of_Template(Document):
    _id = IntField(required=True, primary_key=True)
